...
const setCtrl = {
  ...
  upsertSetting: async (req, res) => {
    try {
      const { id } = req.params;
      const { min, max, category, status } = req.body;
      const name = req.body.name ? req.body.name.trim() : "";
      ...
      if (isDuplicate) {
        return res.status(409).json({ 
          msg: `'${name}' already exists in the ${category} list.` 
        });
      }
      ...
      if (err.code === 11000) {
        return res.status(409).json({ msg: "This entry already exists in the database." });
      }
  ...
  getByCategory: async (req, res) => {
    try {
      const { category } = req.params;
      const data = await Setting.find({ category, status: "Active" }).sort("name");
      res.json(data);
    } catch (err) {
  ...
  getAllSettingsAdmin: async (req, res) => {
    try {
      ...
      const data = await Setting.find().sort("category name");
      res.json(data);
    } catch (err) {
      res.status(500).json({ msg: "Fetch Admin Error: " + err.message });
    }
  },
  ...
  deleteSetting: async (req, res) => {
    try {
      ...
      const deleted = await Setting.findByIdAndDelete(id.trim());
      if (!deleted) return res.status(404).json({ msg: "Setting not found." });
